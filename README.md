...
访问 `/dashboard` 打开监控控制台，包含 8 个监控面板：

| 面板 | 说明 |
|------|------|
| **全局状态栏** | 系统状态、总请求数、平均延迟、错误率、重试次数、配额倒计时、Gemini/Redis 健康 |
| **模型配额** | 各模型已用/限额进度条、平均延迟、错误率、配额耗尽预测 |
| **请求时间线** | 24 小时请求量折线图（鼠标悬停显示详情）+ 昨天对比 |
| **模型路由分布** | 各模型使用量水平条形图（Top 10），含延迟和错误率 |
| **来源分布** | API Key SHA-1 指纹 + 客户端类型（Postman/curl/Python 等）+ 脱敏 IP |
| **错误日志** | 最近 50 条错误，4xx/5xx 分色标注 + 中文解释 + 客户端信息 |
| **最近请求** | 实时请求流，含模型、状态码、延迟、重试次数、客户端类型 |
| **HTTP 状态码速查** | 所有出现过的状态码及中文含义 |
| **运行节点** | 当前 Vercel Edge Function 状态（延迟、请求数、区域） |
